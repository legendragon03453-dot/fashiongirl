      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {categories.map((cat, i) => (
          <FadeIn key={cat.name} delay={i * 100}>
            <Link href="#" className="relative block h-32 md:h-48 overflow-hidden group bg-black">
              <img 
                src={cat.image} 
                alt={cat.name} 
              ...
              <div className="absolute inset-0 flex items-center justify-center p-4 text-center">
                <h3 className="text-white text-xl md:text-2xl font-black uppercase tracking-widest">{cat.name}</h3>
              </div>
            </Link>
          </FadeIn>
        ))}
